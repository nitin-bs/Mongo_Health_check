...
def sub_entity_type_metrics():
    try:
        subentity_job_type_data=mongodb.jobs.aggregate( [ { "$project": { "jobType": 1,"status":1, "days":{"$divide":[{ "$subtract": [datetime.datetime.now(), "$createdAt" ] },600000] }}} ,{"$match":{'days':{'$lte':180}}},{'$group':{"_id":{'jobType':"$jobType",'status':"$status"},"count": { '$sum': 1 }}}] )
        print("\n\n\n\n")
